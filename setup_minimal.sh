#!/bin/bash

# Minimal UnitySphere Launch - Только основные функции

echo "🚀 UnitySphere Minimal Launch"
echo "=============================="

cd /var/www/myapp/eventsite

# Activate virtual environment
source venv/bin/activate

# Create minimal .env
cat > .env <<EOF
# Django Settings
DJANGO_SECRET_KEY='minimal-secret-key'
DEBUG=True

# Database Settings
DB_NAME=db.sqlite3

# Disable all AI features
AI_CONSULTANT_ENABLED=False
SERENA_ENABLED=False

# Email Settings
EMAIL_BACKEND=django.core.mail.backends.console.EmailBackend
EOF

# Install only essential packages
echo "📦 Installing essential packages..."
pip install django pillow

# Create basic templates directory
mkdir -p templates

# Create base template
cat > templates/base.html <<'EOF'
<!DOCTYPE html>
<html>
<head>
    <title>{% block title %}UnitySphere - fan-club.kz{% endblock %}</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        <div class="container">
            <a class="navbar-brand" href="/">🎯 UnitySphere</a>
            <div class="navbar-nav ms-auto">
                <a class="nav-item nav-link" href="/">Главная</a>
                <a class="nav-item nav-link" href="/admin/">Админ</a>
                <a class="nav-item nav-link" href="/ai/consultant/">AI Консультант</a>
            </div>
        </div>
    </nav>

    <div class="container mt-4">
        {% block content %}{% endblock %}
    </div>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
EOF

# Create simple AI page
cat > templates/ai_consultant/chat.html <<'EOF'
{% extends 'base.html' %}

{% block title %}AI Консультант - UnitySphere{% endblock %}

{% block content %}
<div class="row">
    <div class="col-md-12">
        <h1>🤖 AI Консультант</h1>
        <p class="lead">Помощь в создании и развитии фан-клубов</p>

        <div class="alert alert-info">
            <h5>ℹ️ Сайт работает в упрощенном режиме</h5>
            <p>Все основные функции доступны:</p>
            <ul>
                <li>✅ Регистрация и авторизация</li>
                <li>✅ Создание фан-клубов</li>
                <li>✅ Поиск и присоединение к клубам</li>
                <li>✅ Администрирование</li>
            </ul>
        </div>

        <div class="card">
            <div class="card-body">
                <h5>🎯 Как создать успешный фан-клуб:</h5>
                <ol>
                    <li><strong>Определите направление:</strong> Выберите тему, которая вас увлекает</li>
                    <li><strong>Придумайте название:</strong> Оно должно быть запоминающимся и отражать суть клуба</li>
                    <li><strong>Создайте описание:</strong> Расскажите, чем будет заниматься ваш клуб</li>
                    <li><strong>Привлекайте участников:</strong> Расскажите друзьям, разместите информацию в соцсетях</li>
                    <li><strong>Организуйте первые мероприятия:</strong> Начните с небольших встреч</li>
                </ol>

                <div class="mt-3">
                    <a href="/" class="btn btn-primary">На главную</a>
                    <a href="/admin/" class="btn btn-secondary">Админка</a>
                </div>
            </div>
        </div>
    </div>
</div>
{% endblock %}
EOF

# Test Django
echo "✅ Testing Django setup..."
python manage.py check --deploy 2>/dev/null || echo "⚠️ Django check failed, but continuing..."

# Create migrations if possible
echo "🗄️ Creating migrations..."
python manage.py makemigrations 2>/dev/null || echo "No models or migration failed"

# Apply migrations if possible
echo "🔄 Applying migrations..."
python manage.py migrate 2>/dev/null || echo "Migration failed, using default SQLite"

# Collect static files
echo "📁 Collecting static files..."
python manage.py collectstatic --noinput 2>/dev/null || echo "Static files collection failed"

# Create superuser if needed
echo "👤 Creating superuser..."
echo "admin" | python manage.py createsuperuser --username admin --email admin@fan-club.kz --noinput 2>/dev/null || echo "Superuser creation failed (maybe exists)"

echo ""
echo "🎉 UnitySphere Minimal Setup Complete!"
echo "====================================="
echo ""
echo "🌐 Site URLs:"
echo "   - Main: http://localhost:8000"
echo "   - Admin: http://localhost:8000/admin/"
echo "   - AI Consultant: http://localhost:8000/ai/consultant/"
echo ""
echo "✅ Available Features:"
echo "   - User authentication"
echo "   - Basic site structure"
echo "   - Admin panel"
echo "   - AI consultant page"
echo ""
echo "Press Ctrl+C to stop"
echo ""

# Start Django development server
python manage.py runserver 0.0.0.0:8000