...
# 🚀 UnitySphere Simplest Start Script (Port 8006)
# Самый простой запуск Django на порту 8006

echo "🚀 UnitySphere Simplest Start Script (Port 8006)"
echo "==============================================="
echo ""
...
# 4. Проверяем порт 8006
echo "🔍 Проверка порта 8006..."
if ss -tln | grep -q ":8006"; then
    echo "⚠️ Порт 8006 занят"
    exit 1
else
    echo "✅ Порт 8006 свободен"
fi

# 5. Запускаем Django
echo "🚀 Запуск Django на порту 8006..."
...
if curl -s http://127.0.0.1:8006/ > /dev/null 2>&1; then
    ...
    if [ "$SITE_STATUS" = "200" ]; then
        echo "✅ Сайт РАБОТАЕТ!"
        echo ""
        echo "🎉 UnitySphere работает на порту 8006!"
        echo ""
...
echo "   • Порт 8006 (обход конфликта портов)"
